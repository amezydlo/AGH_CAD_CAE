...
% MY CHANGES
function bitmap_h_error(filename,elementsx,elementsy,maxerror,color_edges_black, e_filename)


errors = [];
number_of_elements = [];
for i=1:10
max_refinement_level = i;
...
O = [double(XX(:,:,1)) double(XX(:,:,2)) double(XX(:,:,3))];
F = [double(RGB(:,:,1)) double(RGB(:,:,2)) double(RGB(:,:,3))];

E = rmse(F(:), O(:));

errors = [errors, E];
number_of_elements = [number_of_elements, total_elements];

disp(E)

end
% END MY CHANGES ERRORS

% display image

% MY CHANGES - SAVING CLIP
%resultsFolder = 'Error';
%resultsFilePath = fullfile(resultsFolder, e_filename);

figure;
plot(number_of_elements, errors, 'LineWidth', 3, 'color', 'red');
axis tight;  
saveas(gcf, e_filename);


%imwrite(RGB, resultsFilePath);
% END MY CHANGES
