...
function bitmap_terrain_error(filename)
...
for i = 1:6
    ne = 2 ^ i;
    number_of_elements = [number_of_elements, ne];
    errors = [errors, bitmap_terrain(filename, ne ,2)];

    disp(errors)

    loglog(number_of_elements, errors)

    log_e = log(errors);
    disp(polyfit(number_of_elements, log_e, 1))
end
...
% % Zapis rzutu z góry
% view(2);
% saveas(gcf, 'top_view.png');
% 
% % Zapis kilku rzutów perspektywicznych
% angles = [30, 45, 60];
% for k = 1:length(angles)
%     view(angles(k), 30);
%     saveas(gcf, sprintf('perspective_view_%d.png', angles(k)));
% end
% 
% 
% % Tworzenie animacji 360-stopniowego obrotu
% frames = 360;
% im = cell(1, frames);
% for idx = 1:frames
%     theta = (idx - 1) * (360 / frames);
%     view(theta, 30);
%     drawnow;
%     frame = getframe(gcf);
